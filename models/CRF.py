...
"""
    ...
    https://blog.csdn.net/qq_27590277/article/details/114770421
    报错，除了lengthmask是long(),其他就bool()
"""
import torch
...
class CRF(nn.Module):
    ...
    def _forward_alg(self, feats, mask):
        ...
        for idx, cur_values in seq_iter:
            ...
            #mask_idx=mask_idx.bool()#改7，不能改
            """ effective updated partition part, only keep the partition value of mask value = 1 """

            masked_cur_partition = cur_partition.masked_select(mask_idx)
            #masked_cur_partition=masked_cur_partition.bool() #改6,不能改
            """ let mask_idx broadcastable, to disable warning """
            mask_idx = mask_idx.contiguous().view(batch_size, tag_size, 1)
            mask_idx=mask_idx.bool()#没用
    ...
    def _viterbi_decode(self, feats, mask):
        ...
        #length_mask = length_mask.bool()#改2  不能改
        """ mask to (seq_len, batch_size) """
        mask = mask.transpose(1, 0).contiguous()
        mask = mask.bool()#改5，没用
        ins_num = seq_len * batch_size
        """ be careful the view shape, it is .view(ins_num, 1, tag_size) but not .view(ins_num, tag_size, 1) """
        feats = feats.transpose(1,0).contiguous().view(ins_num, 1, tag_size).expand(ins_num, tag_size, tag_size)
        """ need to consider start """
    ...
    def _score_sentence(self, scores, mask, tags):
        ...
        #length_mask = length_mask.bool()#改3  不能改
        ...
        tg_energy = tg_energy.masked_select(mask.bool().transpose(1, 0))#绝了！就是这
        #=tg_energy.bool()#修改
